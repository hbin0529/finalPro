...
<body>
 ...
 <div class="container" style="width:1300px; margin:50px auto; font-family: 'Pretendard-Regular';">
   ...
   <form id="updateForm" method="post" action="update.bo" enctype="multipart/form-data">
    <input type="hidden" name="boardNo" value="${ h.boardNo }"> <!-- 어떤 걸 삭제할지는 히든으로숨겨서 번호 넘겨주어야함~~~~ -->
 
  	<div class="form-group"> 
      <img src="${path}/resources/img/write.png" width="30px">필수 정보 입력  &ensp;&ensp;
        <span><strong>공간을 이해하는데 필요한 정보이니 최대한 꼼꼼하게 입력해주세요.</strong></span>
     </div>         
      ...
      <div class="form-group"> 
        <label for="homeOption">주거형태</label>&ensp;&ensp;
           <select name="homeOption" style="width: 220px; height: 40px; border: 1px solid rgb(218, 218, 218); border-radius: 4px; color: rgb(161, 160, 160); padding-left: 10px;">
         ...
         <div class="form-group"> 
     <label for="upfile">메인 이미지 첨부</label>&ensp;&ensp;
        <input type="file" name="reupfile" class="main_file">                                       
		    <c:if test="${ not empty h.boardMainimgOrigin}">                    
		 현재 업로드된 파일 : ${ h.boardMainimgOrigin }
		<input type ="hidden" name="originName" value="${ h.boardMainimgOrigin}">
		<input type ="hidden" name="changeName" value="${ h.boardMainimgChange}"> 
		 </c:if>
     </div>
     
         <div class="form-group">  
			<input type="text" class="form-control" id="title" name="boardTitle"  value="${ h.boardTitle }"  placeholder="제목을 입력하세요" style="border-radius:5px; width:100%; padding:10px;">
		</div>
     ...
     <div class="pt-1 text-right" style="text-align: right; margin-top:20px;">
		 <button type="submit" class="btn btn-primary" style="cursor: pointer;  width:10%; padding:13px; font-family: 'Pretendard-Regular';background-color: #21d9cb; font-size: 16px; border: 1px solid white; color: white; border-radius: 5px;" >수정하기</button>
